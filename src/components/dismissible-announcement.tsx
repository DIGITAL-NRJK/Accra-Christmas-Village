...
export function DismissibleAnnouncement({ title, body }: DismissibleAnnouncementProps) {
  ...
  return (
    <div className="overflow-hidden border-b border-acv-gold/35 bg-acv-night text-white">
      <div className="mx-auto flex w-full max-w-6xl items-start gap-3 px-4 py-3 text-sm sm:items-center sm:px-6 lg:px-8">
        <span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-md bg-acv-gold text-acv-night sm:mt-0">
          <Megaphone aria-hidden="true" className="size-4" />
        </span>
        <p
          className="min-w-0 flex-1 leading-6 text-white/82"
      ...
      <div className="h-1 w-full acv-route-band" />
    </div>
  );
}
