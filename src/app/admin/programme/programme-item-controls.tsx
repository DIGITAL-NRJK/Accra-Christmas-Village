...
import {
  ...
  updateProgrammePublicationAction,
} from "@/app/admin/programme/actions";

type ProgrammeItemControlsProps = {
  eventId: string;
  published: boolean;
  title: string;
};
...
export function ProgrammeItemControls({ eventId, published, title }: ProgrammeItemControlsProps) {
  function confirmDelete(event: FormEvent<HTMLFormElement>) {
    if (!window.confirm(`Delete "${title}" from the programme?`)) {
  ...
  return (
    <div className="flex flex-wrap gap-3 border-t border-slate-200 pt-4">
      <form action={updateProgrammePublicationAction}>
      ...
      <form action={deleteProgrammeItemAction} onSubmit={confirmDelete}>
        <input name="eventId" type="hidden" value={eventId} />
        <SubmitButton
