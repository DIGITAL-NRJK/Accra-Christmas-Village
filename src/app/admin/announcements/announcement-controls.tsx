...
import {
  ...
  updateAnnouncementPublicationAction,
...
export function AnnouncementControls({ announcementId, published, title }: AnnouncementControlsProps) {
  function confirmDelete(event: FormEvent<HTMLFormElement>) {
    if (!window.confirm(`Delete "${title}" from announcements?`)) {
  ...
  return (
    <div className="flex flex-wrap gap-3 border-t border-slate-200 pt-4">
      <form action={updateAnnouncementPublicationAction}>
      ...
      <form action={deleteAnnouncementAction} onSubmit={confirmDelete}>
        <input name="announcementId" type="hidden" value={announcementId} />
        <SubmitButton
